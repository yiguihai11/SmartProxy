...
| `Stack` | `stack` | `"gvisor"` | 协议栈实现（默认推荐 `"gvisor"`，移动端及全平台保持成熟稳定；可选支持 `"system"`、`"mixed"` 或实验性 `"go"`） |
...
| `UDPInTCP` | `udp_in_tcp` | `false` | 仅 `socks5`/`socks5h` 有效：选中后 UDP 走 hev UDP-in-TCP（hev-socks5-server 的私有 CMD=5 FWD_UDP 扩展），UDP 报文封装在该节点的 TCP 连接里分帧传输，节点无需 UDP 监听。**因为承载 TCP 流是明文帧（GFW 可识别），该节点 TCP 默认手动关闭**（手动 pin 下，`EffectiveMode` 为 `udp_only`，TCP 路由跳过它）；用户可在节点卡片上手动打开 TCP 电路（`enable`/`auto`）后恢复为 `tcp_and_udp`。UDP 路由恒走帧路径，绝不落到裸 UDP 快路径。也可用 URL 的 `?udp_in_tcp=1` 查询参数表达（导入链接）。面板 Add/Edit Proxy 模态框底部复选框编辑此项 |
...
| `StaticRecords` | `static_records` | `[]` | 静态 DNS 解析记录（hosts 覆盖）：`[{host, ip}]`；`ip` 可写单个地址或数组 `["1.2.3.4", "::1"]`，同一 `host` 多条记录或一条多地址都会把 v4/v6 归到同一 host。查询按类型应答：`A`→v4 记录、`AAAA`→v6 记录、`ANY`→两者、未配的族回 NODATA（空应答让客户端回落）。优先生效于拦截规则与缓存。改配置后热重载生效，无需重启 |

> 域名被封时回填的 IP 已硬编码（IPv4 `0.0.0.0`、IPv6 `::`），不再由 `blocked_ip` / `blocked_ipv6` 配置。
>
> `static_records` 示例：`"static_records": [{"host": "smartproxy.lan", "ip": ["192.168.1.1", "::1"]}]` —— 让面板访问主机名 `smartproxy.lan` 直接解析到路由器局域网地址（v4+v6），无需在路由器上额外配 dnsmasq/hosts（需设备 DNS 查询流经 SmartProxy 才生效）。
