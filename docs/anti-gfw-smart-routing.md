...
* **一票降级与看门狗掐断（已落地实现）**：
  ...
  * 若在看门狗超时时间（默认 5 秒，支持 `smart_proxy.watchdog_timeout_ms` 配置）内未收到远端有效应用层回包，或在握手/首请求早期捕获到 `ECONNRESET` / `broken pipe`：
    1. 立即将对应 `domain` 与 `host` 记入动态黑名单（TTL 3600s）。
    2. 主动向客户端发送真实 **TCP RST**（通过 `netutil.ResetConn(client)` 与 `SO_LINGER=0`）。
    3. 客户端（浏览器、curl、git）在 5 秒内收到 RST 快速失败，现代浏览器与 CLI 立即触发重试；重试连接命中动态黑名单，毫秒级直接切入代理通道成功访问！
...
| 特性 / 机制 | 静态硬编码规则 (旧方案) | 简单首字节探测 (旧方案) | 智能看门狗 TCP RST (当前已落地) | 下一代对冲架构 (规划中) |
| :--- | :--- | :--- | :--- | :--- |
| **规则维护成本** | 极高，需持续手工扩充 `acl.txt` | 无需维护规则 | **零维护**，自主拓扑动态拉黑 | **零维护**，自适应学习 |
| **对抗 GFW 延迟假连**| 有效（完全绕开直连） | **必然翻车（首字节放行后假死90秒）** | **自愈恢复（5秒内 RST 掐断 + 动态黑名单切代理）** | **完全免疫（50ms 对冲无缝接盘）** |
| **首包响应延迟** | 最优（直接命中代理） | 差（多次探测超时） | **优（初次 5s 快速掐断，后续 0 延迟命中代理）** | **优（50ms 对冲竞速，快速选路）** |
| **网络异常自愈** | 无法自愈，未覆盖域名持续卡顿 | 无法自愈 | **自动拉黑降级，客户端快速重试自愈** | **自动熔断惩罚，连接无感透明回放** |
| **内存与连接开销** | 极低 | 低 | **极低（早衰看门狗成功后立即自解脱 disarm）** | 中（需维持短期轻量窥探缓冲） |
...
2. **第二阶段（本次已实施）**：落地了**中继早衰看门狗与 TCP RST 快速恢复机制**（`relay.WithWatchdog`）：
   * 对智能直连通道在首请求阶段设置 5 秒（可配置）看门狗；
   * 一旦发生 GFW 静默黑洞丢包或早期 RST，看门狗毫秒级发出 TCP RST 掐断客户端并拉入黑名单，彻底消除 90 秒假死现象；
   * 客户端重试时命中黑名单，直接无缝走代理。
