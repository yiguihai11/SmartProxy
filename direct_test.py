...
def test_direct_socks5(target_domain, target_ip, port, proxy_host="127.0.0.1", proxy_port=1081):
    """直接测试上游代理1081端口"""
    try:
        # 创建SOCKS5连接
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        sock.settimeout(5)
        print(f"直接连接到上游代理 {proxy_host}:{proxy_port}...")
...
if __name__ == "__main__":
    # 直接测试上游代理
    print("=== 直接测试上游代理1081端口 ===")
    result = test_direct_socks5("cp.cloudflare.com", "104.16.133.229", 80)
    print(f"结果: {result}")

    print("\n=== 直接测试上游代理1081端口443 ===")
    result = test_direct_socks5("cp.cloudflare.com", "104.16.133.229", 443)
    print(f"结果: {result}")